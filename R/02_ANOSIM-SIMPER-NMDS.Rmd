...
```{r}
# from goeveg package
dimcheckMDS(sqrt_spp, k = 10, autotransform = FALSE)
```
...
Put ellipses around the habitat groups and add arrows of SIMPER species.  
...
```{r}
ggord(mds3, expl$habitat_type, poly = FALSE, max.overlaps = 20, var_sub = c(spps))
```

Look at all combinations of the 3 axes:

```{r}
ggord(mds3, expl$habitat_type, axes = c("1", "2"), ellipse = FALSE, max.overlaps = 20, var_sub = c(spps))

ggord(mds3, expl$habitat_type, axes = c("2", "3"), ellipse = FALSE, max.overlaps = 20, var_sub = c(spps))

ggord(mds3, expl$habitat_type, axes = c("1", "3"), ellipse = FALSE, max.overlaps = 20, var_sub = c(spps))
```
...
```{r}
# really should generate a color paletter from r color brewer or something;
# panicking here at the end of the day
mds3_pts2 <- mds3_pts %>% 
    mutate(color = case_when(habitat_type == "seagrass" ~ "green",
                             habitat_type == "shell midden" ~ "brown",
                             habitat_type == "beach" ~ "orange",
                             habitat_type == "erosional edge" ~ "blue",
                             TRUE ~ "red"))
