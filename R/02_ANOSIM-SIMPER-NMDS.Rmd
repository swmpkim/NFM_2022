...
ggord(mds3, expl$habitat_type, axes = c("1", "2"), ellipse = FALSE, max.overlaps = 20, var_sub = c(spps), 
      xlims = c(-1.5, 1.5), ylims = c(-1.5, 1.5))
...
legend3d("right", legend = pal$habitat_type, pch = 16, col = pal$color, ncol = 1, cex = 0.8, inset = c(0.02), text.width = NA)
...
```{r}
# library(vegan3d)
# ordirgl(mds3, groups = expl$habitat_type, col = 1:5)
# orgltext(mds3, display = "species")
...
fig <- plot_ly(mds3_pts2, x = ~MDS1, y = ~MDS2, z = ~MDS3, 
               color = ~habitat_type, colors = pal$color, 
               sizes = c(3, 3)) %>% 
    add_markers()
